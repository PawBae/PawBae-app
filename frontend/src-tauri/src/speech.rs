...
pub fn start_recording() -> Result<(), String> {
    ...
    let tx = SPEECH_TX
        .get()
        .ok_or("Speech thread not initialized")?;
    tx.lock()
        .unwrap()
...
fn speech_thread_main(app: tauri::AppHandle, rx: mpsc::Receiver<SpeechCommand>) {
    ...
    loop {
        ...
        if let Some(ref state) = recording {
            ...
            if should_stop {
                log::info!("[voice] auto-stop: elapsed={}s silence={}s", elapsed, silence_elapsed);
                if let Some(state) = recording.take() {
                    do_stop_recording(state);
                    VOICE_ACTIVE.store(false, Ordering::SeqCst);
                    let _ = app.emit(
...
static LAST_RESULT_TICK: std::sync::atomic::AtomicU64 =
    std::sync::atomic::AtomicU64::new(0);

static RECORDING_START_MS: std::sync::atomic::AtomicU64 =
    std::sync::atomic::AtomicU64::new(0);
...
fn do_start_recording(app: &tauri::AppHandle) -> Result<RecordingState, String> {
    unsafe {
        let speech_cls = AnyClass::get(c"SFSpeechRecognizer")
            .ok_or("SFSpeechRecognizer not available")?;
        ...
        let engine_cls =
            AnyClass::get(c"AVAudioEngine").ok_or("AVAudioEngine not available")?;
        ...
        if let Some(av_audio_app) = AnyClass::get(c"AVAudioApplication") {
            ...
            if !shared.is_null() {
                ...
                log::info!("[voice] mic recordPermission = {} (0=undetermined, 1=denied, 2=granted)", mic_status);
            }
        }
        ...
        let result_handler = RcBlock::new(
            move |result: *mut AnyObject, error: *mut AnyObject| {
                if !result.is_null() {
                    ...
                    let text_ns: *mut AnyObject =
                        msg_send![&*best, formattedString];
        ...
        let tap_block =
            RcBlock::new(move |buffer: *mut AnyObject, _time: *mut AnyObject| {
        ...
        let started: bool =
            msg_send![&*engine, startAndReturnError: &mut err_obj];
        if !started {
            ...
            let err_msg = if !err_obj.is_null() {
                let desc: *mut AnyObject =
                    msg_send![&*err_obj, localizedDescription];
