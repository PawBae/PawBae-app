...
- **设置开关默认关闭**（隐私默认）：前端 `voiceEnabled` 与后端 `VOICE_ENABLED` 均默认
  ...
  延迟到达的 final transcript 在关闭后不分类、不回话、不加好感；已入队的 Start 若在执行前
  被禁用也会被丢弃（不开麦）。
- **权限请求时机：用户显式启用后，而非 app 启动时**。`init_speech_thread()` 不再请求语音识别
  授权；只有在用户开启"语音互动"（`set_voice_enabled(true)`）或首次真正开始录音时才请求，
  已授权/拒绝时为 no-op（不重复弹窗）。新用户默认关闭，启动时不会看到系统授权弹窗。
