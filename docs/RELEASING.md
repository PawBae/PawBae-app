...
```bash
pnpm bump 0.3.0       # tauri.conf.json / package.json / Cargo.toml / Cargo.lock 一次改齐
pnpm bump --check     # 只校验四处一致（tag 构建的 preflight 跑的就是它）
```

Never edit the version in a single file by hand — the four copies drift, and
tag builds hard-fail when the copies disagree or don't match the tag name.
...
to the release, and generates a ready-made updater manifest — `latest.json`
with all three slots and per-asset signatures — attached to the release and
printed in full in its Step Summary (see §4 and "Updater signing" below).

Tag builds refuse to start while the four version copies disagree or don't
match the tag (`pnpm bump --check`), while the updater public key is still
the placeholder, or while the `MINISIGN_SECRET_KEY` secret is missing —
provision the keypair first (see "Updater signing"). macOS tag builds
additionally refuse to start while any of the six Apple signing secrets is
missing (see "macOS signing & notarization").
...
`apps/desktop/src-tauri/src/commands/update.rs`. The release's `latest.json`
asset (generated by the `sign` job) already contains the version, asset URLs,
and per-asset signatures — download it, **fill in `notes_i18n`** (CI leaves
them empty), and deploy it to that path (website repo today;
`apps/website/static/update/latest.json` after the Vercel cutover). Deploying
the manifest is the release valve — nothing reaches users until you do it.
...
build the manifest by hand: use the `latest.json` generated by the `sign` job
(release asset + Step Summary). Verification-aware app versions refuse to
install an asset whose signature is missing or wrong, so a manifest without
signatures effectively disables updates.
