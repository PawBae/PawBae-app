...
- **孵化**：蛋需 `EGG_HATCH_WARMTH = 8` 点暖香破壳。暖香来源：agent 真完工 +1（复用 handleTaskComplete 的真完工判定——ESC/killed 会话不算；且与金币奖励共用同一 60s 会话冷却，被去重的重复 Stop 事件不焐蛋，Codex review），喂食 +1（pet 模式用户的唯一路径，喂食本身 -5 金币）。暖香封顶于阈值。
