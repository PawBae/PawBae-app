...
describe('Social Home window flow', () => {
  ...
  it('mounts an honest local-only model and hides the mini UI while Home is open', () => {
    ...
    expect(mainSource).toContain('friends: []');
    expect(mainSource).toContain('pendingVisit: null');
