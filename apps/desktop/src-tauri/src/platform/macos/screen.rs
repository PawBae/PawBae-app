//! Screen geometry helpers: notch offset, Dock rect, frontmost app window cache.
...
pub(super) unsafe fn cf_string_from(s: &str) -> cg_window::CFStringRef {
...
pub(super) unsafe fn cf_dict_get_double(
    dict: cg_window::CFDictionaryRef,
    key: &str,
) -> Option<f64> {
...
/// fails.
pub(super) unsafe fn cf_dict_get_i32(dict: cg_window::CFDictionaryRef, key: &str) -> Option<i32> {
    let key_cf = cf_string_from(key);
    if key_cf.is_null() {
        return None;
    }
...
pub(super) unsafe fn cf_dict_get_string(
    dict: cg_window::CFDictionaryRef,
    key: &str,
) -> Option<String> {
    let key_cf = cf_string_from(key);
