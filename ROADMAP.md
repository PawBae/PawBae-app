...
### 2.2 Token 花费感知化（已实现核心：Token 喂养循环）

Agent 完成的工作转译成宠物喂食，把 FinOps 变成低认知成本的“喂食”隐喻。

- Agent 真正完成任务时宠物开饭：便宜操作是小零食，贵操作是大餐（零食 ≥2k / 正餐 ≥60k / 大餐 ≥300k tokens，input + output，不计 cache）。
- 设计红线：永不惩罚——token 消耗不扣饥饿度，闲置只有原有的缓慢衰减（照护倒置：干活就是养宠）。
- 不足一顿零食的小任务会累积，攒够再吃。
- 只在本地计算，不上传成本信息。
- 待做：每日预算提醒（宠物举牌）。

实现：纯 reducer `token-feed.ts`（基线水位 + token delta，内存态、启动时预热）+ `pet.svelte.ts` 在 `claude-task-complete` 上结算，数据源复用 `get_claude_stats`。详见 `docs/superpowers/specs/2026-07-07-token-feeding-loop-design.md`。
